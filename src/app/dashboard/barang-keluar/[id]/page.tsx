...
import { ArrowLeft, Printer, Trash2, PackageMinus, Calendar, FileText, User } from "lucide-react";
...
export default function DetailBarangKeluarPage({ params }: { params: Promise<{ id: string }> }) {
  const resolvedParams = use(params);
  const router = useRouter();
  const { data: session } = useSession();
  const [data, setData] = useState<OutgoingGoodsDetail | null>(null);
  const [loading, setLoading] = useState(true);
  const [deleting, setDeleting] = useState(false);

  const fetchData = async () => {
    try {
      const res = await fetch(`/api/barang-keluar/${resolvedParams.id}`);
  ...
  const handleDelete = async () => {
    if (!confirm("Hapus transaksi barang keluar ini? Stok barang terkait akan dikembalikan.")) return;
    setDeleting(true);
    try {
      const res = await fetch(`/api/barang-keluar/${resolvedParams.id}`, { method: "DELETE" });
      const json = await res.json();
      if (json.success) {
        toast.success("Transaksi berhasil dihapus dan stok barang dikembalikan");
    ...
    <div className="flex flex-col gap-4 sm:gap-6 max-w-5xl mx-auto w-full print:p-0 print:max-w-none">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 print:hidden">
        <div className="flex items-center gap-3 sm:gap-4">
          <Button variant="ghost" size="icon" className="h-8 w-8 sm:h-9 sm:w-9" onClick={() => router.push("/dashboard/barang-keluar")}>
            <ArrowLeft className="h-4 w-4 sm:h-5 sm:w-5" />
          </Button>
          <div>
            <div className="flex items-center gap-2">
              <h1 className="text-xl sm:text-2xl font-bold tracking-tight">{data.number}</h1>
              <span className="px-2.5 py-0.5 rounded-full text-xs font-semibold uppercase bg-amber-500/10 text-amber-600 border border-amber-500/20">
                {data.type}
              </span>
            </div>
            <p className="text-xs sm:text-sm text-muted-foreground">
              Dokumen Pengeluaran / Penghapusan / Mutasi Barang
            </p>
          </div>
        </div>
      ...
      <div className="bg-card print:bg-white text-card-foreground print:text-black rounded-xl border print:border-none shadow-sm p-4 sm:p-6 lg:p-8 space-y-6 overflow-x-auto">
        ...
        <div className="text-center my-4">
          <h1 className="text-xl font-bold uppercase underline tracking-wide">
            BERITA ACARA PENGELUARAN / MUTASI BARANG
          </h1>
          <p className="text-xs text-muted-foreground print:text-gray-700 mt-1">Nomor Dokumen: {data.number}</p>
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-4 p-4 rounded-lg bg-muted/40 print:bg-gray-50 border text-sm">
          <div>
            <span className="text-xs text-muted-foreground block">Tanggal Pengeluaran:</span>
            <span className="font-semibold">{formatDate(data.date)}</span>
          </div>
          <div>
            <span className="text-xs text-muted-foreground block">Jenis Pengeluaran:</span>
            <span className="font-semibold uppercase">{data.type}</span>
          </div>
          <div>
            <span className="text-xs text-muted-foreground block">Tujuan / Penerima:</span>
            <span className="font-semibold">{data.destination || "Gudang Afkir / Mutasi"}</span>
          </div>
          <div>
            <span className="text-xs text-muted-foreground block">Petugas Penanggung Jawab:</span>
        ...
        {data.note && (
          <div className="p-3 bg-muted/30 border rounded text-xs">
            <span className="font-bold">Catatan / Alasan Khusus:</span> {data.note}
          </div>
        )}

        {/* Table of items */}
        <div>
          <h4 className="font-bold text-base mb-3">Daftar Barang yang Dikeluarkan</h4>
            ...
            <Table>
              <TableHeader>
                <TableRow className="bg-muted/50 print:bg-gray-100">
                  ...
                  <TableHead>Alasan Pengeluaran</TableHead>
              ...
              <TableBody>
                {data.items.map((item, idx) => (
                  <TableRow key={item.id}>
                    ...
                    <TableCell className="text-center font-bold text-sm text-red-600 print:text-black">
                      {item.quantity} Unit
                    </TableCell>
                ...
                <TableRow className="bg-muted/30 print:bg-gray-50 font-bold">
                  <TableCell colSpan={3} className="text-right text-sm">TOTAL BARANG KELUAR:</TableCell>
                  <TableCell className="text-center text-sm font-extrabold text-red-600 print:text-black">
                    {totalQuantity} Unit
                  </TableCell>
                  <TableCell colSpan={2} />
                </TableRow>
        ...
        <div className="pt-8 grid grid-cols-2 gap-8 text-center text-sm">
          <div className="flex flex-col items-center">
            <p className="text-xs text-muted-foreground print:text-gray-700">Petugas Pengeluar (Toolman),</p>
            <div className="h-20" />
            <p className="font-bold underline uppercase">{data.user?.name || "....................................."}</p>
            <p className="text-xs text-muted-foreground">Pengelola Lab RPL</p>
          </div>
