...
export default function KategoriPage() {
  ...
  const handleDelete = async (id: string, cName: string) => {
    if (!confirm(`Hapus kategori ${cName}?`)) return;
    try {
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
        <div>
          <h1 className="text-xl sm:text-2xl font-bold text-foreground">Kategori Barang</h1>
          <p className="text-muted-foreground text-xs sm:text-sm">Klasifikasi jenis perangkat dan peralatan laboratorium</p>
        </div>
        <button
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        {loading ? (
          ...
          <div className="col-span-full py-12 text-center text-muted-foreground">Belum ada kategori.</div>
        ) : (
          categories.map((c) => (
            <div
              key={c.id}
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              <div>
                ...
                <input
                  ...
                  placeholder="Contoh: Storage & Penyimpanan"
                  required
